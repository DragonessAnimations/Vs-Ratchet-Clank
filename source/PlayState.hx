...
class PlayState extends MusicBeatState
{
	...
	var lightbgback:FlxSprite;
	var lightbgfront:FlxSprite;
	...
	override public function create()
	{
		...
		switch(SONG.song.toLowerCase())
		{
			...
			case 'metropolis' | 'assault' :
			{
					curStage = 'kerwan';
					var bg:FlxSprite = new FlxSprite(-250, -200).loadGraphic(Paths.image('kerwanlightbgback'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.2, 0.2);
					bg.active = false;
					bg.setGraphicSize(Std.int(bg.width * 0.8));
					...
					var kerwanFront:FlxSprite = new FlxSprite(-350, -550).loadGraphic(Paths.image('kerwanlightbgfront'));
					kerwanFront.setGraphicSize(Std.int(kerwanFront.width * 1.1));
					kerwanFront.updateHitbox();
					kerwanFront.antialiasing = true;
					kerwanFront.scrollFactor.set(0.9, 0.9);
					kerwanFront.active = false;
					add(kerwanFront);
			}
			case 'kick-ass' :
			{
					curStage = 'kerwandark';
                    var kerwanTex = Paths.getSparrowAtlas('kerwan_smoke');
					var bg:FlxSprite = new FlxSprite(-250, -450).loadGraphic(Paths.image('kerwan_smoke'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.2, 0.2);
					bg.active = true;
					bg.frames = kerwanTex;
					bg.animation.addByPrefix('idle', 'Smoke0');
					bg.animation.play('idle');
					bg.setGraphicSize(Std.int(bg.width * 0.8));
					bg.updateHitbox();
					add(bg);

					var kerwanFront:FlxSprite = new FlxSprite(-350, -550).loadGraphic(Paths.image('kerwandarkbgfront'));
					kerwanFront.setGraphicSize(Std.int(kerwanFront.width * 1.1));
					kerwanFront.updateHitbox();
					kerwanFront.antialiasing = true;
					kerwanFront.scrollFactor.set(0.9, 0.9);
	...
	function newIntro(?dialogueBox:DialogueBox):Void
	{
		...
		var bigboom:FlxSprite = new FlxSprite();
		bigboom.frames = Paths.getSparrowAtlas('bigboom_Assets', 'shared');
		bigboom.animation.addByPrefix('idle', 'BIGBOOM0', 24, false);
		bigboom.setGraphicSize(Std.int(bigboom.width * 6));
		bigboom.scrollFactor.set();
		bigboom.updateHitbox();
		bigboom.screenCenter();
